...
async function getSiteId(token: string, siteUrl: string): Promise<string> {
  ...
  const data = await res.json();
  if (!data.id) throw new Error(`Site não encontrado: ${JSON.stringify(data)}`);
  return data.id;
}
...
async function uploadFile(
  ...
  // Upload simples (até 4MB) via PUT
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!tenantId || !clientId || !clientSecret || !siteUrl) {
      return new Response(
        JSON.stringify({ success: false, error: "Credenciais SharePoint não configuradas" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // Verificar se é teste de conexão
    const contentType = req.headers.get("content-type") || "";
    if (contentType.includes("application/json")) {
      const body = await req.json();
      if (body.action === "test") {
        // Testar apenas autenticação Azure + acesso ao site
    ...
    if (!file || !cronogramaItemId || !codigoCliente) {
      return new Response(
        JSON.stringify({ success: false, error: "Campos obrigatórios: file, cronograma_item_id, codigo_cliente" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
